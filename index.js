...
client.connect(err => {
  const moviesCollection = client.db("myCineplex").collection("movies");
  console.log('database connected');

  const registersCollection = client.db("myCineplex").collection("registers");

  app.post('/addMovies', (req, res) => {
    const movies = req.body;
    console.log(movies)
    moviesCollection.insertOne(movies)
    .then(result => {
